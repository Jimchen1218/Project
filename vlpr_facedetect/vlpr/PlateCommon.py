...
def random_scene(img, data_set):
    ...
    #print("random_scene data_set:", data_set)
    datasets_len = len(data_set)
    #print("random_scene datasets_len:", datasets_len)
    if datasets_len == 0:
        print("data_set is null")
        return None, None
    bg_img_path = data_set[r(datasets_len)]
    # print bg_img_path
    ...
    print("random_scene env.shape[0]:", env.shape[0]," env.shape[1]:", env.shape[1])
    print("random_scene img.shape[0]:", img.shape[0]," img.shape[1]:", img.shape[1])
...
def GenCh1(f,val):
    ...
    draw = ImageDraw.Draw(img)
    draw.text((0, 2),chr(val),(0,0,0),font=f)
    A = np.array(img)
    return A
